...
const sendSlackNotification = async (prefix, entry, apiChanges = null) => {
  ...
  const blocks = [
    {
      ...
      text: {
        type: "mrkdwn",
        text:
          `*🆕 New Chrome Version: ${entry.version}*\n` +
          `*Channel:* ${channel}\n` +
          `*Date:* ${date}`,
      },
    },
  ...
  if (apiChanges && apiChanges.browserApis) {
    const apiChangesText = [
      "*📊 API Changes:*\n",
      `*✨ Added (${apiChanges.browserApis.added.length})*`,
      apiChanges.browserApis.added.length > 0
        ? apiChanges.browserApis.added.map((api) => `• ${api}`).join("\n")
        : "",
      `\n*🗑️ Removed (${apiChanges.browserApis.removed.length})*`,
      apiChanges.browserApis.removed.length > 0
        ? apiChanges.browserApis.removed.map((api) => `• ${api}`).join("\n")
        : "",
    ].join("\n");
  ...
  for (const [i, entry] of list) {
    ...
    if (previousVersion) {
      await (async function () {
        ...
        const previousFilePath = getCDPProtocolFilePath(
          prefix,
          previousVersion
        );
    ...
    if (previousVersion) {
      ...
      // Send the diff directly as apiChanges
      const apiChanges = {
        browser: prefix,
        version: {
      ...
      // Only send notification here
      await sendSlackNotification(prefix, entry, apiChanges);
